...
export default function StatsSection({ data }: Props) {
   ...
   * =========================================================
   *
   * Stat hanya ditampilkan kalau number ATAU label diisi
   * di CMS. Kosongkan keduanya di CMS untuk menyembunyikan
   * satu box stat, tanpa perlu field ACF tambahan.
   */
   ...
   *
   * Dua kondisi:
   * 1. Toggle manual dari CMS (ACF True/False: hide_and_show)
   *    Checked = SEMBUNYIKAN blok Stat Number + Label.
   * 2. Auto-hide kalau semua stat kosong.
   *
   * Default (belum dicentang / field belum dibuat) = tetap
   * tampil, supaya behavior lama gak berubah.
   */

  const manuallyHidden = Boolean(data?.hide_and_show);

  const hasStats = stats.length > 0;

  const showStatsBlock = !manuallyHidden && hasStats;
