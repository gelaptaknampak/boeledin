...
export default async function AboutCoreServicesPage({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string | string[] }>;
}) {
  const config = aboutSectionConfig.coreServices;
  ...
  console.log("=== ABOUT CORE SERVICES PAGE ===");
  console.log("rawLang:", rawLang);
  console.log("lang:", lang);

  // Ambil ID About Core Services sesuai bahasa
  ...
  if (!postId) {
    throw new Error(
      `Post About Core Services untuk bahasa ${lang} belum dikonfigurasi`,
    );
  }
  ...
  if (!post) {
    throw new Error(`About Core Services untuk bahasa ${lang} tidak ditemukan`);
  }
