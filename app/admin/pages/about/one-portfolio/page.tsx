...
export default async function AboutOnePortfolioPage({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string | string[] }>;
}) {
  const config = aboutSectionConfig.onePortfolio;
  ...
  console.log("=== ABOUT ONE PORTFOLIO PAGE ===");
  console.log("rawLang:", rawLang);
  console.log("lang:", lang);

  // Ambil ID About One Portfolio sesuai bahasa
  ...
  if (!postId) {
    throw new Error(
      `Post About One Portfolio untuk bahasa ${lang} belum dikonfigurasi`
    );
  }
  ...
  if (!post) {
    throw new Error(
      `About One Portfolio untuk bahasa ${lang} tidak ditemukan`
    );
  }
