...
export default async function AboutIndustriesWeServePage({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string | string[] }>;
}) {
  const config = aboutSectionConfig.industriesWeServe;
  ...
  console.log("=== ABOUT INDUSTRIES WE SERVE PAGE ===");
  console.log("rawLang:", rawLang);
  console.log("lang:", lang);

  // Ambil ID About Industries We Serve sesuai bahasa
  ...
  if (!postId) {
    throw new Error(
      `Post About Industries We Serve untuk bahasa ${lang} belum dikonfigurasi`,
    );
  }
  ...
  if (!post) {
    throw new Error(
      `About Industries We Serve untuk bahasa ${lang} tidak ditemukan`,
    );
  }
